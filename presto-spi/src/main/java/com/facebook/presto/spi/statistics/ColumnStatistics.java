...
public final class ColumnStatistics
{
    ...
    {
        // We add 10 % overestimate to account for inaccurate estimate by below method. It is okay to slightly overestimate than underestimate.
        float overEstimatePercentage = 0.10f;
        return stringRange.map(value -> {
            ...
            return (long) (estimate + estimate * overEstimatePercentage);
        }).orElse(OPTION_SIZE);
    }
